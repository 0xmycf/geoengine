...
use geoengine_operators::source::gdal_source::process::{setup_client, setup_client_for_bytes};
use libgdalprocess::{
    construct_tile,
    ipc_channel_service::{self, SendType, SimpleIpcChannelMessage},
    random_data,
};
            ...
            loop {
                ...
                if serialize_on_iteration {
                    the_data = std::hint::black_box(
                        to_vec8(std::hint::black_box(tile.clone()))
                            .expect("Failed to convert tile to IPC data"),
                    );
                }
                match msg {
