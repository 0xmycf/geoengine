...
use geoengine_datatypes::{
    dataset::NamedData,
    util::{ByteSize, helpers::json_schema_help_link},
};
...
impl OperatorSchema {
        ...
        T: JsonSchema + OperatorName,
    {
        Self {
            r#gen: || {
                ...
                let mut schema = r#gen.into_root_schema_for::<T>();
                let obj = schema.as_object_mut().unwrap(); // TODO! handle this unwrap gracefully
                // TODO same as above
                match obj.get("title").map(|f| f.as_str().unwrap_or("")) {
                ...
                // TODO handle the unwrap gracefully
                // also validate if this is equivalent to the source
                match obj
                    .get_mut("properties")
                    .map(|o| o.as_object_mut().unwrap())
                {
                    Some(props) => {
                        props.insert(
                            "type".to_owned(),
                            serde_json::json!(T::TYPE_NAME.to_owned()),
                        );
                    }
                    _ => {}
                }
    ...
    fn get_schema(&self) -> serde_json::Value {
        ...
        let mut schema = (self.r#gen)();

        let (link_str_key, link_obj_value) = json_schema_help_link(self.help_url);
        let mut map = Map::new(); // TODO is this the best approach?
        let obj = schema.as_object_mut().unwrap_or(&mut map);
        obj.insert(link_str_key, link_obj_value);
